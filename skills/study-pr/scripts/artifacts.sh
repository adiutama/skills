...
# Overrides:
#   AGENTS_ARTIFACTS_ROOT=/absolute/path
#   AGENTS_ARTIFACTS_SCOPE=local|global
#   ARTIFACT_GITHUB_OWNER + ARTIFACT_GITHUB_REPO (must be supplied together)
...
artifact_resolve_branch_base() {
  ...
  local rel
  rel="$(_artifact_repo_rel "$owner" "$repo" "$branch_slug")"
...
_artifact_resolve_owner_repo() {
  local url
  local override_owner="${ARTIFACT_GITHUB_OWNER:-}"
  local override_repo="${ARTIFACT_GITHUB_REPO:-}"

  if [[ -n "$override_owner" || -n "$override_repo" ]]; then
    if [[ -z "$override_owner" || -z "$override_repo" ]]; then
      printf 'ARTIFACT_GITHUB_OWNER and ARTIFACT_GITHUB_REPO must be supplied together.\n' >&2
      return 1
    fi
    if [[ ! "$override_owner" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ || ! "$override_repo" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
      printf 'Invalid GitHub owner or repository for artifact path.\n' >&2
      return 1
    fi
    OWNER="$override_owner"
    REPO="${override_repo%.git}"
    return
  fi
