...
> /work-until lint is clean in `src/` — no public API changes, max 8.

Init → confirm → work → verify lint on each `done`.
...
> /work-until workspace review session shows 0 unresolved findings and `npm test` exits 0.
...
> /work-until latest blast-radius session verdict is `safe to commit`.
