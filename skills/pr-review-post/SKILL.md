---
name: pr-review-post
description: Post findings from a saved review session as inline GitHub PR comments. Use after /pr-review to publish findings and submit an approve/request-changes review.
...
Invoke as `/pr-review-post [<pr-url or pr-number>] [optional free-text instructions]`.
...
1. **Current session**: if `/pr-review` already ran, reuse `OWNER`, `REPO`, `NUMBER`, `HEAD_SHA`, `SESSION_PATH`.
