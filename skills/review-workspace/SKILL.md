...
description: Skeptical pre-push review of local git changes — catches shippability issues before the PR. Session under .agents/artifacts/.../review-workspace/ when gitignored, else ~/.agents/artifacts/ (see `<SKILL_DIR>/scripts/artifacts.sh` allocate).
